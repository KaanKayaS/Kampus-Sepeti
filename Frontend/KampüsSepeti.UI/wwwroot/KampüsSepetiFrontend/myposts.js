window.deletePost = function (postId) {
    console.log("Silme başlatıldı:", postId); // Debug için
    if (confirm("Bu ilanı silmek istediğinize emin misiniz?")) {
        // API'ye DELETE isteği gönderiyoruz
        fetch(`/MyPosts/RemovePost?id=${postId}`, { // Burada URL parametresi doğru şekilde postId ile oluşturuluyor
            method: "DELETE",
        })
            .then(response => {
                if (response.ok) {
                    alert("İlan başarıyla silindi.");
                    // DOM'dan silinen öğeyi kaldırıyoruz
                    const postElement = document.querySelector(`.post-card[data-id='${postId}']`);
                    if (postElement) {
                        postElement.remove();
                    }
                } else {
                    alert("Bir hata oluştu. İlan silinemedi.");
                }
            })
            .catch(error => {
    ...
    cities.forEach(city => {
        ...
        document.querySelectorAll('.delete-btn').forEach(button => {
            button.addEventListener('click', function () {
                ...
                deletePost(postId);  // deletePost fonksiyonunu çağır
            });
        });
